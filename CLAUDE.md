...
- **Sectors:** A01 Memory Grid (M00–07) · A02 Deep Network (M08–20).
...
Codes **Not started** · AI Integration **v1 — NEVA Core Assistant (advisory, backend-only)** ·
...
pnpm server    # waitlist API + NEVA Core AI (Node, :8787). Dev: run alongside `pnpm dev` (Vite proxies /api). Prod: serves dist + /api.
pnpm waitlist:count  # print local waitlist totals (no emails)
```
...
`pnpm server` auto-loads `.env`. Never commit `.env` or log the key.
...
| `server/` | Backend (zero-dep Node, `pnpm server`). `index.mjs` = http server: `POST /api/waitlist`, `GET /api/waitlist/stats` (no emails), **`POST /api/ai/neva-core`** (NEVA Core hints), serves `dist/` in prod. `waitlist-store.mjs` = validation + dedupe + `data/waitlist.json` fs store (git-ignored). `neva-ai.mjs` = **NEVA Core Assistant v1** — OpenAI Responses API via `fetch` (no SDK), advisory hint text only, deterministic offline fallback, `OPENAI_API_KEY`/`OPENAI_MODEL` server-side only. `scripts/waitlist-summary.mjs` = `pnpm waitlist:count`. No DB, no auth, no accounts/invite-code flow yet. |
...
| `src/network.ts` | Builds the **interactive graph once at module load** → `NETWORK`. `INTERACTIVE_COUNT=220` nodes in `CLUSTER_COUNT=11` lobes; nearest-neighbour `edges`/`neighbours` (≤ `MAX_NEIGHBOURS=3`, within `LINK_DISTANCE=30`); per-node `meta` (id/type/status/signal/access). |
